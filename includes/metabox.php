...
if ( "003" === $location_meta['error_code'] || empty( $location_meta['error_code'] ) && $location_meta['lat'] && $location_meta['lng'] ) {
	...
	echo '<img class="field-trip-map-view" src="http://maps.googleapis.com/maps/api/staticmap?zoom=13&size=600x300&maptype=roadmap&markers=color:blue%7Clabel:S%7C' . urlencode( $location_meta['lat'] ) . ',' . urlencode( $location_meta['lng'] ) .'&sensor=false" />';
